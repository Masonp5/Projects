...
#define ENDLINE_STRING "\r" //the endline string used to identify end of transmission. Lines in the cdc_acm_user_ev_handler function also must be changed
...
#define DEVICE_NAME                     "Brooks Smart Cradle"                      /**< Name of device. Will be included in the advertising data. */
...
#define EXTERNAL_BUTTON                 NRF_GPIO_PIN_MAP(0,24)    //mapping button to P0.24
#define MAIN_LED                        NRF_GPIO_PIN_MAP(1,10)    //mapping LED to P1.10
...
static void peer_manager_init(void)
{
    ...
    // Security parameters to be used for all security procedures for pairing and bonding
...
/*ble_gap_sec_keyset_t keyset;          //function used for making keyset if security is desired for bluetooth connection
void init_keyset(void){ 
        
            
       keyset.keys_own.p_enc_key = NULL;
                                    ...
                                    app_usbd_cdc_acm_user_event_t event)
{
    ...
    switch (event)
    {
        ...
        case APP_USBD_CDC_ACM_USER_EVT_RX_DONE:
        {
            ...
            do
            {
                if ((m_cdc_data_array[index-1] == '\r') || //must match end of transmission character
...
int main(void)
{
    ...
    peer_manager_init();
    //init_keyset();  //used for making security params if desired
    ble_stack_init();
    gap_params_init();
    gatt_init();
    services_init();
    ...
    nrf_gpio_cfg_output(MAIN_LED);	//configuring gpios
    nrf_gpio_cfg_input(EXTERNAL_BUTTON, NRF_GPIO_PIN_NOPULL);
    
    // Enter main loop.
    for (;;){
        
        //nrf_gpio_pin_toggle(MAIN_LED);
        if(connected == 1){
            nrf_gpio_pin_clear(MAIN_LED); //set led pin on when connected
        }
        else if(connected == 0 ){ //if ble not connected
                nrf_gpio_pin_toggle(MAIN_LED);//toggle led
            }
                
    
        if(nrf_gpio_pin_read(EXTERNAL_BUTTON)==0){
            //nrf_gpio_pin_clear(MAIN_LED); //used for debugging
